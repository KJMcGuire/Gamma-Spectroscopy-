...
###Subtract global and local background Ra-226 attenuated and unattenuated spectra
...
perr = np.sqrt(np.diag(abs(ra_params1)))   #Gives 1 standard deviation error of fit parameters
...
##################
#Define test data to check functions
##################


def test_gaussian(x, sigma, X):
...
count_uncertainty = count_uncert(ra_params1[0], ra_params1[2], perr[0,0], perr[2,2])
...
delta_mu_1_25mm = del_mu(sum2, sum1, 1.25, 3888, 4586, 0.5)
...
plt.plot(x, Radium_7mm_lead, color = '#fec615', label="6.7 mm attenuator")
