...
sigma_y = sigma_y(calibration_take_2.b, calibration_take_2.m, calibration_take_2.energy_calibration1[1], calibration_take_2.energies)
...
delta = delta(calibration_take_2.energy_calibration1[1])
sigma_b = sigma_b(sigma_y, calibration_take_2.energy_calibration1[1], delta)
sigma_m = sigma_m(sigma_y, calibration_take_2.energy_calibration1[1], delta)
...
def res(sigma, E):
    res = 0.0
    sum = 0.0
    for i in range ( len(calibration_take_2.energy_calibration1[0])):
        res = (2.*abs(sigma[i])*100.*calibration_take_2.m)/E[i]
        print res
#        resolutions.fill(res())
        sum += res
    print (sum/ [len(calibration_take_2.energy_calibration1[0])])
